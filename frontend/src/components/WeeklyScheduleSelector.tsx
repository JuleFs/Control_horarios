...
const WeeklyScheduleSelector: React.FC<WeeklyScheduleSelectorProps> = ({
  ...
  const handleAgregarHorario = () => {
    ...
    const grupo = grupos.find(
      (g) => g.ID_Grupo.toString() === formData.Grupo_ID
    );
    const materia = materias.find(
      (m) => m.ID_Materia.toString() === formData.Materia_ID
    );
    const salon = salones.find(
      (s) => s.ID_Salon.toString() === formData.Salon_ID
    );
  ...
  const handleRemoveHorario = (id: number) => {
    setHorariosAgregados(
      horariosAgregados.filter((horario) => horario.id !== id)
    );
  };
  ...
  const handleGuardarHorarios = () => {
    ...
    const horariosParaAPI = horariosAgregados.map((horario) => {
      ...
      // Calculamos la diferencia de días para llegar al día deseado
      let diffDays = horario.diaIndex - currentDayOfWeek;

      // Si la diferencia es negativa, sumamos 7 para ir a la próxima semana
      // en lugar de ir a la semana anterior
      if (diffDays < 0) {
        diffDays += 7;
      }
      ...
      console.log(
        `Día seleccionado: ${horario.diaNombre} (índice ${horario.diaIndex})`
      );
      console.log(
        `Fecha calculada: ${format(
          targetDate,
          "yyyy-MM-dd"
        )} (día de semana: ${targetDate.getDay()})`
      );
      ...
      <Box sx={{ width: "100%" }}>
          ...
          >
            <Box sx={{ flex: 1, minWidth: "200px" }}>
              <TimePicker
                ...
                onChange={(newValue) =>
                  handleTimeChange("HoraInicio", newValue)
                }
                  ...
                  {horariosAgregados
                    ...
                    .map((horario) => (
                      <TableRow key={horario.id}>
                        ...
                        <TableCell>
                          {horario.HoraInicio.substr(0, 5)} -{" "}
                          {horario.HoraFin.substr(0, 5)}
                        </TableCell>
        ...
        {horariosAgregados.length === 0 && (
          <Alert severity="info" sx={{ mb: 4 }}>
            No hay horarios agregados. Complete el formulario y presione
            "Agregar Horario" para comenzar.
          </Alert>
        )}
      </Box>
